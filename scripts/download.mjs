...
const childprocess = await createChildProcess({ rpc: { port: 6800 }, environment: 'ignore' });
const client = await createClient(childprocess);
await attachWebUI(childprocess, { port: 6801 });

console.log('Web UI: http://127.0.0.1:6801?port=6800&secret=' + childprocess.getOptions().secret);
